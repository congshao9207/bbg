...
class TransFlowRawData(TaskBaseExecutor):
    ...
    #         data['_balance_for_merge'] = data.get('account_balance', pd.Series([None] * len(data))).fillna('__NULL__')
    #         existing_df['_balance_for_merge'] = existing_df.get('account_balance',
    #                                                             pd.Series([None] * len(existing_df))).fillna('__NULL__')
    ...
    #     )
    #
    #     # 找出匹配成功的行（_merge == 'both'），并提取它们对应的原始 index
    #     repeated_indices = merged_result[merged_result['_merge'] == 'both']['index']
    #     # 使用 .loc 根据原始 index 精准打标签
    #     data.loc[repeated_indices, 'repeated'] = 1
    #
    #     # 7. 清理临时列
